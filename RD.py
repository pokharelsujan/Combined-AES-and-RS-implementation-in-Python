...
def rs_correct_errata(msg_in, synd, err_pos): # err_pos is a list of the positions of the errors/erasures/errata
    ...
    for i, Xi in enumerate(X):
        ...
        for j in range(0, Xlength):
            if j != i:
                err_loc_prime_tmp.append( en.gf_sub(1, en.gf_mul(Xi_inv, X[j])) )
        # compute the product, which is the denominator of the Forney algorithm (errata locator derivative)
        err_loc_prime = 1
        for coef in err_loc_prime_tmp:
            err_loc_prime = en.gf_mul(err_loc_prime, coef)
        ...
        y = en.gf_poly_eval(err_eval[::-1], Xi_inv) # numerator of the Forney algorithm (errata evaluator evaluated)
        y = en.gf_mul(gf_pow(Xi, 1), y)
        
        # Check: err_loc_prime (the divisor) should not be zero.
        if err_loc_prime == 0:
            print("Could not find error magnitude")    # Could not find error magnitude

        # Compute the magnitude
        magnitude = en.gf_div(y, err_loc_prime) # magnitude value of the error, calculated by the Forney algorithm (an equation in fact): dividing the errata evaluator with the errata locator derivative gives us the errata magnitude (ie, value to repair) the ith symbol
...
def rs_find_errata_locator(e_pos):
    ...
    for i in e_pos:
        e_loc = en.gf_poly_mul( e_loc, en.gf_poly_add([1], [en.gf_pow(2, i), 0]) )
    return e_loc
...
def rs_find_error_evaluator(synd, err_loc, nsym):
    ...
    _ , remainder = en.gf_poly_div( en.gf_poly_mul(synd, err_loc), ([1] + [0]*(nsym+1)) ) # first multiply syndromes * errata_locator, then do a
...
def rs_calc_syndromes(msg, nsym):
    ...
    for i in range(0, nsym):
        synd[i] = en.gf_poly_eval(msg, en.gf_pow(2,i))
...
'''def rs_forney_syndromes(synd, pos, nmess):
    ...
    for i in range(0, len(pos)):
        x = en.gf_pow(2, erase_pos_reversed[i])
        for j in range(0, len(fsynd) - 1):
            fsynd[j] = en.gf_mul(fsynd[j], x) ^ fsynd[j + 1]
...
def rs_find_error_locator(synd, nsym, erase_loc=None, erase_count=0):
    ...
    for i in range(0, nsym-erase_count): # generally: nsym-erase_count == len(synd), except when you input a partial erase_loc and using the full syndrome instead of the Forney syndrome, in which case nsym-erase_count is more correct (len(synd) will fail badly with IndexError).
        ...
        for j in range(1, len(err_loc)):
            delta ^= en.gf_mul(err_loc[-(j+1)], synd[K - j]) # delta is also called discrepancy. Here we do a partial polynomial multiplication (ie, we compute the polynomial multiplication only for the term of degree K). Should be equivalent to brownanrs.polynomial.mul_at().
        ...
        if delta != 0: # Update only if there's a discrepancy
            if len(old_loc) > len(err_loc): # Rule B (rule A is implicitly defined because rule A just says that we skip any modification for this iteration)
                ...
                new_loc = en.gf_poly_scale(old_loc, delta)
                old_loc = en.gf_poly_scale(err_loc, en.gf_inverse(delta)) # effectively we are doing err_loc * 1/delta = err_loc // delta
                err_loc = new_loc
            ...
            err_loc = en.gf_poly_add(err_loc, en.gf_poly_scale(old_loc, delta))
    ...
    if (errs-erase_count) * 2 + erase_count > nsym:
        print("Too many errors to correct")    # too many errors to correct
...
def rs_find_errors(err_loc, nmess): # nmess is len(msg_in)
    ...
    for i in range(nmess): # normally we should try all 2^8 possible values, but here we optimize to just check the interesting symbols
        if en.gf_poly_eval(err_loc, en.gf_pow(2, i)) == 0: # It's a 0? Bingo, it's a root of the error locator polynomial,
...
def rs_correct_msg(msg_in, nsym, erase_pos=None,only_erasures=False):
    ...
    # prepare the syndrome polynomial using only errors (ie: errors = characters that were either replaced by null byte or 
    synd = rs_calc_syndromes(msg_out, nsym)
    print("syndrome v1 :",synd)
    ...
    err_pos = rs_find_errors(err_loc, len(msg_out)) # find the roots of the errata locator polynomial (ie: the positions of the errors/errata)
    if err_pos is None:
        print("Could not locate error")
    ...
    print("synd: ",synd,"\n")
    if max(synd) > 0:
        print("Could not correct message")
